...
| Backend | Supabase (Auth + Postgres). **All 16 public tables** (load/save behaviour in Data persistence below): `faculty_state` (JSONB blob) · exams: `exams`, `exam_results` · students: `students`, `student_batches`, `student_attendance`, `students_meta`, `student_logins` · insights: `class_reports`, `student_plans` · events: `lecture_absences`, `homework_pending`, `exam_absences` · quiz: `quizzes`, `quiz_attempts` · `teacher_feedback` (superadmin-RLS). **Column-level schema for all 16 tables: [`DATABASE_SCHEMA.md`](./DATABASE_SCHEMA.md)**; behavioural detail (write paths, send flows) in the Data persistence section below + [`FLOWS.md`](./FLOWS.md). |
