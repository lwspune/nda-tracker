...
- **Prod online admin** (Vercel): Four Supabase stores:
  ...
  - `lecture_absences (id uuid, lws_id FK ON DELETE CASCADE, date text, subject text, created_at, created_by)` — UNIQUE `(lws_id, date, subject)`, indexed on `date` and `(lws_id, date)`. Sparse event log: one row per (student, date, subject) the student missed (NOT a full per-slot grid). Faculty enters via the Lecture log tab on the Attendance page; replace-set semantics per "period card" via `lectureAbsenceSlice.setLectureAbsenteesForPeriod`. RLS: authenticated only.
...
| `api/send-lecture-absences.js` | Vercel serverless — verifies admin JWT, loops `students[]` from request body (each with `subjects[]`), Wabridge template variables `[name, date, subjects.join(', ')]`. Requires `WABRIDGE_LECTURE_MISS_TEMPLATE_ID` env. Skips students with empty subjects. Same dev shim pattern as above. |
...
| `src/store/slices/lectureAbsenceSlice.js` | `setLectureAbsenteesForPeriod(date, subject, lwsIds)` (replace-set: delete then insert) / `getLectureAbsencesForDate(date)` / `getLectureAbsencesForStudent(lwsId, sinceDate?)`. Tags inserts with `created_by` from the auth session email. |
...
| `src/pages/Attendance/LateMarkingWidget.jsx` | Admin-only top-of-Attendance widget. Search + chip list + "Send Morning Late Notifications" button. Each add/remove writes/deletes a `status='L'` row via `markLate`/`unmarkLate`. |
| `src/pages/Attendance/LectureLogTab.jsx` | Date + batch pickers → per-period cards from `getTodaysLectures`. Click "Mark absentees" → `MarkAbsenteesModal`. Send button passes `absencesByLwsId` + `date` up to parent. |
| `src/pages/Attendance/MarkAbsenteesModal.jsx` | Searchable multi-select scoped to one (date, subject). Search filters visible list but does NOT drop previously-checked students from the saved set. Save calls parent's `onSave(lwsIds)`. |
| `src/pages/Attendance/LateNotificationPreviewModal.jsx` | Pre-send modal for late: editable mobile + parent_mobiles per student, redirect-to test field, Confirm/Cancel. |
| `src/pages/Attendance/LectureMissPreviewModal.jsx` | Pre-send modal for lecture-miss: same shape as LateNotification but each row shows the subjects-missed list, forwarded to the endpoint as `subjects[]`. |
...
Test files mirror source paths under `__tests__/`. Python tests under `tests/`. **820 Vitest tests passing**. **39 Python tests** in `tests/test_subtopic_merge.py`.
Key coverage: analytics filters, GAT routing, tag validation, dashboard filters, Exams/Students/StudentView pages, re-upload modals, mergeStudents (incl. dedup signals, exam-name candidates, `addNameVariant`), split script, send_schedule (44 tests), timetableSlice (44 tests), configSlice (36 tests), studentSlice (6 tests), insightsSlice + insightsSupabase (21 tests covering save/clear dual-path + table helpers), persist.js (Supabase load/save/pagination), useStore loadExamsFromSupabase action, Exams pagination (11 tests), attendance parse (8 tests), attendanceSlice (20 tests covering import + L-protection + markLate/unmarkLate/getLateStudentsForDate), AttendanceRings (6 tests), student-login login tracking + lecture-absences (2+ tests), consecutiveAbsent (14 tests), migrate_insights (11 tests), subtopic rename (39 Python tests), `getTodaysLectures` pure helper (12 tests), `lectureAbsenceSlice` (15 tests), `LateMarkingWidget` (9 tests), `MarkAbsenteesModal` (9 tests), `LectureLogTab` (12 tests), `LateNotificationPreviewModal` (6 tests), `LectureMissPreviewModal` (5 tests), `RecentIncidents` (6 tests), `send-late-notifications` endpoint (8 tests), `send-lecture-absences` endpoint (5 tests).
