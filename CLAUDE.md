...
- **Superadmin** (Vercel + Supabase): Online Admin **plus** the Teacher Feedback page. Supabase account with `user_metadata.role='superadmin'`; routes through the admin portal, gated by the `isSuperadmin` flag + `teacher_feedback` RLS. See "Teacher Feedback (superadmin-only)".
...
- **Prod online admin** (Vercel): Four Supabase stores:
  ...
  - `quizzes` + `quiz_attempts` — Daily Quiz (deliberately separate from `exams`). Written by `quizSlice.js`/`quizSupabase.js`; read via `loadQuizzesFromSupabase()`. Stripped from the JSONB blob in `saveToSupabase`, kept in `saveToStorage` (dev disk). See "Daily Quiz" for the full schema + flow.
...
Test files mirror source paths under `__tests__/`. Python tests under `tests/`. **1304 Vitest tests passing** (2026-06-05, full `vitest run`: 82 files). **39 Python tests** in `tests/test_subtopic_merge.py`. Recent additions: Daily Quiz (quiz lib 24, quizStats 13, quizSlice + quizSupabase, `student-quizzes`/`quiz-submit` endpoints, `QuizLinkPage`); pending-aware notifications (late/lecture/homework widget + modal pending coverage). For test-infrastructure detail (mock patterns, growth log, chainable Supabase builder) see memory `project_testing.md`.
