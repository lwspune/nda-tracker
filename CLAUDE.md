...
- **Hostel & mess attendance** (APJ boarders, Phase 1 — 2026-07-08) — exception-capture (default-present) for 5 checkpoints (`hostel_am`/`breakfast`/`lunch`/`dinner`/`hostel_pm`), mirroring `lecture_absences`. Roll checkpoints add a **reconciliation gate** (`checkpoint_confirmations`; `reconciled=false` → open incident). `leaves` explains absences across every checkpoint in its window. The daily **chain** (`src/lib/analytics/chain.js`, pure) composes hostel/mess + derived class attendance to flag unexplained "fell off the chain" boarders. Admin-only **Hostel & Mess** tab in `src/pages/Attendance/HostelTab.jsx`; slices `checkpointSlice.js` + `leavesSlice.js`; scoped to `branch='APJ'`. Tables + flow: [`DATABASE_SCHEMA.md`](./DATABASE_SCHEMA.md) §3b + [[project_hostel_attendance]].
