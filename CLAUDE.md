...
**Days-late badge on AttendanceRings** — below each monthly donut, when the month has any `L` rows, a small yellow chip reads `Days late: N`. Clicking expands an inline `DD MMM` list (latest first); single-month-at-a-time toggle. Months that have only L rows still render a ring (P/A denominator = 0 → 0%, but the badge surfaces the absence info anyway).
...
| `src/pages/Attendance/AttendanceRings.jsx` | SVG donut rings per calendar month (R=40, stroke-dasharray arc); sorted latest-first; rendered inside `StudentView` (below exam data), visible in all three portals. Below each ring, conditional yellow `Days late: N` chip — clickable to expand inline list of `DD MMM` dates (latest first, single-month toggle). Months with only L rows still render a ring at 0%. |
...
- `lecture_absences` `UNIQUE (lws_id, date, subject)` is load-bearing for the replace-set semantics in `setLectureAbsenteesForPeriod`. Removing it would let duplicate rows accumulate per period.
...
- `AttendanceRings` `Days late: N` chip is gated on `lateCount > 0` for each month; expanded state is single-month-at-a-time (`expandedMonth` is a `string | null` at the `AttendanceRings` level, not per-ring). Do not move state into `Ring` — that would lose the auto-collapse-on-open-another behaviour.
