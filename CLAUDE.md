...
- **Offline results** (hand-graded, total marks only) → exam with `questions:[]` + explicit `maxMarks` (`exams.max_marks`). Offline-ness is derived (`!exam.questions?.length`); `examMaxMarks(exam)` is the single %-of-max denominator. Per-question analytics are intentionally empty (UI shows "Offline"). Absence-flagging defaults **off** for offline. **Two entry paths, both converging on the same student-row shape** (`OfflineExamModal`): the **in-app marks grid** (default — roster derived from the selected batches via `buildOfflineRoster`, one number input per student, optional paste-a-column; pure helpers in `src/lib/offlineRoster.js`) and the **`Name`/`Marks` xlsx upload** (kept for bulk/legacy sheets; `buildOfflineTemplateRows(rosterNames)` pre-fills the Name column). A **blank mark = did not appear** — the row is omitted, which is what feeds absentee flagging. The grid excludes `Block`/`Quit`/`Inactive` students (`isBlockedStatus`) so they never surface as phantom absentees; recording one needs the file path.
