...
Test files mirror source paths under `__tests__/`. Python tests under `tests/`. **1192 Vitest tests passing** (2026-06-04, full `vitest run`: 70 files; +22 homeworkSlice, +8 send-homework-pending, +2 RecentIncidents homework chip). **39 Python tests** in `tests/test_subtopic_merge.py`. For test-infrastructure detail (mock patterns, growth log, chainable Supabase builder) see memory `project_testing.md`.
