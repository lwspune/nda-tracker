...
`syllabusBatches[]` and `timetables[].batchName` are kept **1:1** by the unified `renameBatch` / `deleteBatch` actions in `configSlice`. The original `migrate_unify_batches.js` (retained for archaeology) produced a 10-name `_A`/`_B` scheme; the user has since reshaped the central lists via Settings → Batches to a 7-batch state (no `_B` sections; some stray spaces inside parens that may or may not be intentional). The unification *contract* (syllabus == timetable, both renamed atomically) is enforced by the slice; the *specific name list* is whatever Settings says today. Verify the current set against Supabase before reasoning about specific batch names.

`profile.batches[]` (student records) is **not** aligned with the central list. The student-portal join key (Phase 3 of the branch-batch plan) will require either renaming profile batches or building a profile→unified map; deferred. See `project_branch_batch_plan` in memory.

`branches[]` (the top-level store key seeded by `seedBranches(saved)` in `defaults.js`) may not be populated in Supabase yet if the deployed bundle predates the configSlice commit — confirmed `null` in `faculty_state.data` at 2026-05-20 14:28. Once an admin opens the latest deploy on a fresh session, the seed runs and `_save()` writes the union into the JSONB.
...
Test files mirror source paths under `__tests__/`. Python tests under `tests/`. **713 Vitest tests passing**. **39 Python tests** in `tests/test_subtopic_merge.py`.
