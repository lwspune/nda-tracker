...
- **Timetable** (`src/pages/Timetable/`) — branch/batch grids, mappings, exam schedule, Subject Hours pivot, batch-tab reorder. **Join-key invariant:** `batchName` is the join key (`examSchedules.batchName`, `student_batches.batch_name`, `exams.batch`); rename via `renameTimetableBatch` (cascades) — a bare `updateTimetable({batchName})` leaves exam schedules stale. `title`/`footnotes` are presentation-only. Excel export uses `xlsx-js-style` (do not revert to `xlsx`).
