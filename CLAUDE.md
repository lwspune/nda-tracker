...
**Page** (`src/pages/TeacherFeedback/`): per-teacher cards (overall score, 8-dimension bars, cross-cycle trend, collapsible raw comments), worst-first, cycle filter. Superadmin-only (`isSuperadmin` guard + RLS).
...
| `src/pages/TeacherFeedback/index.jsx` | Superadmin-only page: per-teacher cards (overall, 8-dim bars, trend, comments), worst-first, cycle filter. Guards on `isSuperadmin`. |
