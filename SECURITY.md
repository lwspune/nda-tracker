...
| **`teacher_feedback`** | Enabled | **Superadmin only** — `(auth.jwt() -> 'app_metadata' ->> 'role') = 'superadmin'`. The **first role-restricted policy** in the project: a normal admin (no role claim) cannot read or write it. This is a real DB-level boundary, not just a UI gate. |
...
Component-level visibility (which features render for which mode) is enforced in the UI via `useMode()` / the `isSuperadmin` store flag and acts as defence-in-depth on top of RLS. **UI gating is not a security boundary by itself** — anyone able to obtain a Supabase session for an authenticated user can read any RLS-permitted table directly. The teacher account is not a security boundary against the admin data — it can read everything authenticated users can read; teacher mode is read-only by UI convention, not by RLS. **`teacher_feedback` was believed to be the exception, then found not to be (2026-08-11), and is now genuinely one (2026-09-10).** The policy originally read `auth.jwt() -> 'user_metadata' ->> 'role'`, and `user_metadata` is **self-editable** by any signed-in user via `supabase.auth.updateUser({ data: { role: 'superadmin' } })` — so a teacher could relabel their own account and walk straight through both the teacher-feedback boundary and the `faculty_state` teacher-write denial. **Migrated to `app_metadata` on 2026-09-10**, which only the Admin API (service-role key) can write, and which therefore never reaches a browser.

The migration was: backfill `raw_app_meta_data.role` from `raw_user_meta_data.role` for all 25 accounts → flip the policies → revoke every refresh token and delete every session (32 live), forcing a global re-login so all JWTs carry the new claim. Client and server each read the claim through **one** module — `src/lib/authRole.js` and `api/_authRole.js` — both of which read `app_metadata` **only, with no fallback**, because a fallback would reinstate the hole for any token that simply omits the block. Provisioning (`api/teacher-account.js`, `create_teacher_account.js`) now writes `app_metadata: { role: 'teacher' }`; `full_name` stays in `user_metadata` as display data, so a teacher editing their own name cannot touch their role.

**The three `faculty_state` deny-lists intentionally still name `user_metadata`, and Supabase's linter still flags them.** They deny when **either** block says `teacher` — a union of deny conditions, which is strictly stronger than either alone. RLS reads the JWT directly and access tokens are stateless, so a sign-out cannot revoke them; a pre-migration token carries no `app_metadata.role`, and under an `app_metadata`-only deny-list `null IS DISTINCT FROM 'teacher'` is **true**, which would have *granted* those teachers blob writes for the token's remaining lifetime. The union closes that window. Tighten to `app_metadata` alone once all pre-migration tokens have expired — see Known gaps.

**Superadmin account provenance:** `vilas11shinde@gmail.com` was created directly in `auth.users` + `auth.identities` via SQL (the local env had no service-role key for the Admin API), mirroring an existing working row, with a bcrypt password and `raw_user_meta_data = {"role":"superadmin"}` — since migrated so the authoritative claim is `raw_app_meta_data.role`. Password resets / future superadmins should go through the Supabase dashboard or the Admin API, not more hand-rolled SQL.
...
| ~~`user_metadata` used in 4 RLS policies~~ | ~~High~~ | **FIXED 2026-09-10** — role claim migrated to `app_metadata`; all 25 accounts backfilled, all sessions revoked, both readers (`src/lib/authRole.js`, `api/_authRole.js`) read `app_metadata` with no fallback. `teacher_feedback` is now a real DB boundary. |
| **Tighten the 3 `faculty_state` deny-lists to `app_metadata` alone** | Low | **OPEN (deliberate, short-lived).** They currently deny on *either* metadata block to cover pre-migration access tokens, which RLS cannot revoke. Supabase's linter still reports 3 `rls_references_user_metadata` ERRORs for this — expected, since it cannot tell a deny-union from a grant. Safe to tighten once all tokens issued before 2026-09-10 have expired; drop the `user_metadata` conjunct from `faculty_write_{insert,update,delete}`. |
