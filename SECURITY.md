...
**The three `faculty_state` deny-lists briefly named `user_metadata` as well, and that was deliberate.** RLS reads the JWT directly and access tokens are stateless, so the global sign-out could not revoke them; a pre-migration token carries no `app_metadata.role`, and under an `app_metadata`-only deny-list `null IS DISTINCT FROM 'teacher'` is **true** — which would have *granted* those teachers blob writes for the token's remaining lifetime. So they denied on **either** block (a union of denies, strictly stronger than either alone) until the longest-lived pre-migration token expired one TTL after the ~10:35 UTC revocation. **Tightened to `app_metadata` alone the same day**, once that window had passed. Supabase's security advisors now report **zero ERROR-level findings**.
...
| ~~Tighten the 3 `faculty_state` deny-lists to `app_metadata` alone~~ | ~~Low~~ | **DONE 2026-09-10**, one TTL after the token revocation. No `rls_references_user_metadata` findings remain; the advisors report zero ERRORs. |
