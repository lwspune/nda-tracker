...
// the standalone shareable link page (QuizLinkPage). Both pass a `mobile` so the
// submit can be attributed to the right student.
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mt-4 sticky bottom-0 bg-bg/95 backdrop-blur py-3">
        ...
        {onCancel && <button className="text-[13px] text-ink-3 hover:text-ink px-3" onClick={onCancel} disabled={submitting}>Cancel</button>}
      </div>
    </div>
  )
}
...
export function QuizReview({ title, review, onBack }) {
  ...
  return (
    <div>
      {onBack && <button className="text-[13px] text-ink-3 hover:text-ink mb-3" onClick={onBack}>← Back</button>}
