...
// q        — question object { q, chapter, subtopic, question, optionA..D, answer, solution }
...
export default function QuestionCard({ q, examId, studentAnswer, studentResult }) {
  ...
  const canEdit    = !IS_READ_ONLY && !!examId
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden bg-surface shadow-sm">
      ...
      <div className="p-4">
        {editing ? (
          <EditMode q={q} onSave={handleSave} onCancel={() => setEditing(false)} />
        ) : hasContent ? (
          <ViewMode
            q={q}
...
// ── Edit Mode ─────────────────────────────────────────────────
function EditMode({ q, onSave, onCancel }) {
