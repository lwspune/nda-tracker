...
| **Teacher** | Production hostname + Supabase session with `user_metadata.role = 'teacher'` | Supabase Auth (email + password), individual accounts | Read-only across cohort, **except** own-lecture attendance capture at `/school-attendance` | Reads `faculty_state` + `exams` + `exam_results` + students; writes `lecture_absences` + `lecture_submissions`. `faculty_state` writes denied by RLS |
...
| Filing board — filed vs outstanding (top of Lecture log tab) | ✓ | — | — |
| `/school-attendance` — own-lecture capture | ✓ (empty unless the admin's email matches a teacher record) | ✓ | — |
| `/hostel-mess-attendance` — warden/mess capture | ✓ (only if flagged) | ✓ only when `hostelAccess` is set on their teacher record | — |
| "Hostel & Mess" sidebar item | — | ✓ only when `hostelAccess` is set | — |
| "My Lectures" sidebar item | — | ✓ | — |
...
| `src/store/slices/submissionSlice.js` | `submitLecture({date, slotId, batchName, subject?, teacherId?, absentCount, source})` (upsert on `(date, slot_id, batch_name)`; stamps `submitted_by` from the session email and `submitted_at` explicitly — the column default never re-applies on the update half) / `getSubmissionsForDate(date)`. |
...
| `src/lib/timetableRestructure.js` | Pure `restructureTimetable(timetable, plan, {mkId})` + `allDays()` — rebuilds a timetable's slot rows and grid from a routine plan. **A surviving slot keeps its id**: grid cells, `lecture_absences`, `lecture_submissions` and `teacher_calendar_blocks.block_key` all key off `slotId`, so recreating one at the same clock time orphans every one of them. `cells:'keep'` carries a teaching slot's subjects through a retime; anything else replaces the row. Throws on an unmatched or twice-matched slot rather than silently minting a new one. Used by `migrate_timetable_routine.js`. |
| `src/lib/absenceTimeBackfill.js` | Pure `planAbsenceTimeBackfill({rows, slotTimes}, {withReport})` — the rows needing their clock time frozen before a slot retime (see CLAUDE.md → Timetable). Skips already-stamped rows (idempotent) and `adhoc_`-prefixed impromptu rows; reports orphaned slot ids rather than inventing a time. Used by `migrate_absence_times.js`. |
...
| `src/pages/Attendance/MarkAbsenteesModal.jsx` | Searchable multi-select scoped to one (date, slot). Search filters visible list but does NOT drop previously-checked students from the saved set. Save calls parent's `onSave(lwsIds)`. Shared by `LectureLogTab` (admin) and `SchoolAttendancePage` (teacher). |
| `src/pages/Attendance/FilingBoard.jsx` | Admin filed-vs-outstanding panel at the top of the Lecture log tab. Reads `getSubmissionsForDate` + `buildFilingBoard`; collapsed = chase-list of `teacher · subject · batch` for outstanding periods, expanded = full table. Renders nothing when the day has no timetabled periods. |
| `src/pages/SchoolAttendance/index.jsx` | The `/school-attendance` teacher surface: own periods for a date, filed/not-filed per card, `MarkAbsenteesModal` → `setLectureAbsenteesForPeriod` then `submitLecture`. Always renders the "Signed in as X · Not you?" line (shared-device protection; falls back to `supabase.auth.signOut()`). Unmatched email → "not linked to a teacher record" empty state. |
| `src/lib/teacherDay.js` | Pure: `findTeacherByEmail` (case/whitespace-forgiving, blank never matches), `getTeacherLecturesForDate` (own periods across batches, time-ordered, **falls closed** on unknown teacher), `withFilingStatus` (pairs lectures with `lecture_submissions`; `filed` is row-based, never count-based), `buildFilingBoard` (admin view — keeps teacher-less periods). |
| `src/lib/routing.js` | `SCHOOL_ATTENDANCE_PATH` / `HOSTEL_ATTENDANCE_PATH` + their `is…Path(pathname, baseUrl)` matchers — the app's only path-shaped entry points (everything else is a query param); strip `BASE_URL` for the gh-pages build. |
| `src/pages/HostelAttendance/index.jsx` | The `/hostel-mess-attendance` warden/mess surface: checkpoint picker, tap-to-cycle exception capture (present → absent → sick → out-pass), on-leave rows locked, headcount reconciliation on roll checkpoints only. Gated on `hasHostelAccess`; dates are DD-MM-YYYY. |
| `src/lib/hostelRoster.js` | Pure `buildBoarderRoster(studentProfiles, branches)` + `HOSTEL_BRANCHES` — shared by the admin Hostel tab and the staff page so the two marking lists can't drift. Excludes variants, non-Active and day-scholars. |
...
| `src/lib/analytics/dashboard.js` | Dashboard aggregates: `examAvgPct`, `getPerformanceSeries`, `getClassProjectedAvg`, `getPriorityChapters`, `getBatchComparison`. All pure; %-of-max is the only score unit. `getPerformanceSeries` + `getAtRisk` are now reached only *through* `getBatchComparison` (their widgets were removed 2026-09-12); **`getPriorityChapters` has no UI consumer at all** — kept pure + tested, see `SUGGESTIONS.md` 2026-09-12. |
...
| `src/pages/Dashboard/index.jsx` | Command-center page (attendance roll-up + leaders + integrity rollup + never-logged-in + batch comparison + hardest-Q). Subject→branch→batch→exam filter chain, which now feeds only the batch comparison and hardest-Q table. |
| `src/pages/Dashboard/BatchComparison.jsx` | Dashboard widget: per-batch comparison table, full-width, rendered only when >1 batch is in scope. (`KpiStrip.jsx` deleted 2026-06-07; `PerformanceTrend.jsx` + `PriorityChapters.jsx` deleted 2026-09-12 along with the inline Chapter-Performance and At-Risk cards.) |
...
| `src/lib/examPdf.js` | `downloadExamPdf(exam)` — jsPDF exam report. Question detail cards are typeset as HTML and placed as images (`captureCardImage`, html2canvas at `CAPTURE_SCALE`); `stripLatex()` + `drawAsciiCard()` are the **per-card fallback** when a capture fails, not the main path |
| `src/lib/examCardHtml.js` | Pure `buildQuestionCardHtml(item, type)` — one question card as self-contained inline-styled HTML at `CARD_WIDTH_PX`, which maps 1:1 onto the printed card width so the capture wraps exactly as the PDF does |
| `src/lib/mathHtml.js` | Pure `renderMathHtml` / `renderRichHtml` — stored question text → HTML with KaTeX. Lifted out of `<Math>` so the PDF's cards share the screen's renderer instead of growing a second one; `renderRichHtml` adds GFM tables (no scroll box — a captured image cannot scroll) |
