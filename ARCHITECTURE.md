...
| `api/send-hostel-alert.js` | Hostel warden alert (Phase 2). Admin-JWT POST (`{dryRun, redirectTo, date}`) + cron-secret branch (not scheduled). Service-role reloads roster (APJ Active residential) + `student_attendance` + `checkpoint_absences` + `leaves` for the date, **re-computes the chain** (`buildDailyChain`), and WhatsApps `buildWardenAlert` output (`[date, listText]`) to `faculty_state.hostelAlertMobiles[]`. Fail-closed on `WABRIDGE_HOSTEL_ALERT_TEMPLATE_ID`; only sends when count>0 and a number exists; stateless. |
