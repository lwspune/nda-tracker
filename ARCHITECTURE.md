...
| `src/lib/routing.js` | `SCHOOL_ATTENDANCE_PATH` + `isSchoolAttendancePath(pathname, baseUrl)` — the app's only path-shaped entry point (everything else is a query param); strips `BASE_URL` for the gh-pages build. |
